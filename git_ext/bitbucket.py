...
class BitbucketRemote(Remote):
    ...
    def submit_new_pr(self, pr):
        ...
        post_data = {
            ...
            'reviewers': [
                {'username': username} for username in pr.reviewers],
            'destination': {
                'branch': {'name': pr.dest_branch}},
            'source': {
                'branch': {'name': pr.source_branch},
                'repository': {'full_name': repository_full_name}},
        }
        logger.info(post_data)
        resp = requests.post(self.pull_requests_url,
                             auth=self.user.auth,
                             json=post_data)
    ...
    def get_activities(self, pr_id):
        resp = requests.get(self.ACTIVITY_DETAIL.format(username=self.repo_username,
                                                        repo_slug=self.repo_name,
